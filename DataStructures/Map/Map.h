...
namespace my
{
    template < typename _Key, typename _Ty, typename _Cmp = std::less<_Key>, typename _Alloc = std::allocator< std::pair<const _Key, _Ty> > > 
    class map
    {
    public:
        ...
        map() : size_(0)
              , capacity_(20)
              , data_(_Alloc().allocate(20))
        {
        }

        map(const _Myt &_Rhs) : size_(_Rhs.size_)
                              , capacity_(_Rhs.size_ + 20)
                              , data_(_Alloc().allocate(_Rhs.size_))
        {
            int count = 0;
